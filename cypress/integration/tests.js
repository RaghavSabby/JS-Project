...
describe("Check All API response", () => {
    it("Check All API response", () => {
        cy.visit("/");
        cy.intercept("GET", "https://restcountries.com/v2/all").as("allCountries");
        
        cy.wait("@allCountries").should((response) => {
            console.log(response);
            expect(response.response.body.length).to.eq(250);
...
describe("Check Search API response", () => {
    it("Check Search API response", () => {
        cy.visit("/");
        cy.intercept("GET", "https://restcountries.com/v2/name/india").as("searchedCountry");
        cy.get("form").within(() => {
            cy.get("input").type("india");
            cy.get("button.search-btn").click();
        });
        cy.wait("@searchedCountry").should((response) => {
            console.log(response);
            expect(response.response.body.length).to.eq(2);
            expect(response.response.statusCode).to.eq(200);
        });
    });
});
...
describe("Check Fiter API response", () => {
    it("Check Fiter API response", () => {
        cy.visit("/");
        cy.intercept("GET", "https://restcountries.com/v2/region/Africa").as("filterCountry");
        cy.get("select#countries").select("Africa");
        cy.wait("@filterCountry").should((response) => {
            console.log(response);
            expect(response.response.body.length).to.eq(60);
            expect(response.response.statusCode).to.eq(200);
        });

        cy.visit("/");
        cy.intercept("GET", "https://restcountries.com/v2/region/Europe").as("filterCountry");
        cy.get("select#countries").select("Europe");
        cy.wait("@filterCountry").should((response) => {
            console.log(response);
            expect(response.response.body.length).to.eq(53);
            expect(response.response.statusCode).to.eq(200);
        });
    });
});
...
describe("Check alpha3Code API response", () => {
    it("Check alpha3Code API response", () => {
        cy.visit("/");
        cy.wait(5000);
        cy.get("main.content-container").within(() => {
            cy.get("article.country-card").then((articles) => {
                cy.intercept("GET", "https://restcountries.com/v2/alpha/ALB").as("wishlistCountry");
                const albaniaCard = articles[2];
                cy.get(albaniaCard).within(() => {
                        cy.get("i").click();
                });
                cy.wait("@wishlistCountry").should((response) => {
                    console.log(response);
                    expect(response.response.body.name).to.eq("Albania");
                    expect(response.response.statusCode).to.eq(200);
                });
            });
        });

        // Page Refresh. Loading from LocalStorage
        cy.visit("/");
        cy.wait(5000);
        cy.intercept("GET", "https://restcountries.com/v2/alpha/ALB").as("wishlistCountry");
        cy.wait("@wishlistCountry").should((response) => {
